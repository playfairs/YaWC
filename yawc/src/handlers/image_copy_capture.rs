...
impl ImageCopyCaptureHandler for State {
    fn image_copy_capture_state(&mut self) -> &mut ImageCopyCaptureState {
        &mut self.yawc.image_copy_capture_state
    }

    fn capture_constraints(&mut self, source: &ImageCaptureSource) -> Option<BufferConstraints> {
        let (size, _scale) = self.yawc.buffer_size_and_scale_for_source(source)?;
    ...
    fn new_session(&mut self, session: Session) {
        ...
        self.yawc.capture_sessions.push(session);
    }
    ...
    fn cursor_capture_constraints(
        ...
        let (size, _scale) = self.yawc.buffer_size_and_scale_for_cursor_source(source)?;
    ...
    fn new_cursor_session(&mut self, session: CursorSession) {
        ...
        self.yawc.cursor_capture_sessions.push(session);
    }
    ...
    fn frame_aborted(&mut self, frame: FrameRef) {
        ...
        for session in self.yawc.capture_sessions.iter() {
            session
                .user_data()
        ...
        for session in self.yawc.cursor_capture_sessions.iter() {
            session
                .user_data()
                .get_or_insert(|| RefCell::new(None::<Frame>))
                .borrow_mut()
    ...
    fn session_destroyed(&mut self, session: SessionRef) {
        self.yawc.capture_sessions.retain(|sess| *sess != session);
    }
...
impl State {
            ...
            .unwrap()
        {
            ImageCaptureSourceKind::Output(output) => {
                ...
                let elements = if session.draw_cursor() {
                    let Some(output_geo) = self.yawc.space.output_geometry(&output) else {
                        frame.fail(CaptureFailureReason::Stopped);
                        return;
                    };

                    let pointer_loc = self.yawc.seat.get_pointer().unwrap().current_location()
            ...
            ImageCaptureSourceKind::Toplevel(foreign_toplevel) => {
                ...
                let elements = if session.draw_cursor() {
                    self.backend
                        .with_renderer(|renderer| {
                            let win_loc = self.yawc.space.element_location(&win);
                            let pointer_elements = if let Some(win_loc) = win_loc {
                                let pointer_loc =
                                    self.yawc.seat.get_pointer().unwrap().current_location()
                                        - win_loc.to_f64()
    ...
    fn process_cursor_capture_session(&mut self, session: CursorSessionRef) {
        ...
        let cursor_offset = self
            .yawc
            .cursor_state
            .cursor_geometry(self.yawc.clock.now(), scale)
            .unwrap_or_default()
            .loc;
    ...
    fn update_cursor_capture_positions(&mut self) {
        ...
        let cursor_loc = self.yawc.seat.get_pointer().unwrap().current_location();
        ...
        for session in self.yawc.cursor_capture_sessions.iter() {
                ...
                .unwrap()
            {
                ...
                ImageCaptureSourceKind::Toplevel(foreign_toplevel) => {
                    ...
                    let Some(window_loc) = self.yawc.space.element_location(&window) else {
                        return;
                    };
                    ...
                    let mut cursor_geo = self
                        .yawc
                        .cursor_state
                        .cursor_geometry(self.yawc.clock.now(), fractional_scale)
                        .unwrap_or_default();
    ...
    fn handle_frame(
        ...
        self.backend.with_renderer(|renderer| {
            ...
            frame.success(Transform::Normal, damage, self.yawc.clock.now());
        });
